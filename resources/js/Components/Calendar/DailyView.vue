...
const checkNextMonthEvents = () => {
    ...
    const nextMonth = getFormattedMonth(new Date(formattedDate.setMonth(formattedDate.getMonth() + 1)).toISOString())
    existNextMonthEvents.value = !!props.days?.calendarView?.hasOwnProperty(nextMonth);
};

const checkPreviousMonthEvents = () => {
    const formattedDate = new Date(filterDate.value);
    const previousMonth = getFormattedMonth(new Date(formattedDate.setMonth(formattedDate.getMonth() - 1)).toISOString());
    existPreviousMonthEvents.value = !!props.days?.calendarView?.hasOwnProperty(previousMonth);
};

const shownMonth = ref(new Date().toISOString().split('T')[0].slice(0, 7))
const getTitleMonth = () => {
    return new Date(filterDate.value).toLocaleDateString(
        'en-US',
        {
            year: 'numeric',
            month: 'long',
...
const scrollMonth = (direction) => {
    const formattedDate = new Date(filterDate.value);
    if (direction === 'next') {
        filterDate.value = new Date(formattedDate.setMonth(formattedDate.getMonth() + 1)).toISOString();
...
onMounted(() => {
    ...
    shownMonth.value = new Date(props.currentDate).toISOString().split('T')[0].slice(0, 7)
        ...
        <div class="isolate flex flex-auto overflow-hidden bg-white">
                                ...
                                <li class="relative mt-px flex"
                                    ...
                                    <a
                                        ...
                                        <p :class="`order-1 font-semibold text-${event.colour}-700`">
                                            {{ event.title }}</p>
                                        <p :class="`text-${event.colour}-500 group-hover:text-${event.colour}-700`">
            ...
            <div class="hidden w-1/2 max-w-md flex-none border-l border-gray-100 px-8 py-10 md:block">
                <div class="flex items-center text-center text-gray-900">
                    <button v-if="existPreviousMonthEvents" type="button" @click="scrollMonth('previous')"
                            class="-m-1.5 flex flex-none items-center justify-center p-1.5 text-gray-400 hover:text-gray-500">
                        <span class="sr-only">Previous month</span>
                        <ChevronLeftIcon class="size-5" aria-hidden="true"/>
                    </button>
                    <div class="flex-auto text-sm font-semibold">{{ getTitleMonth() }}</div>
                    <button v-if="existNextMonthEvents" type="button" @click="scrollMonth('next')"
                            class="-m-1.5 flex flex-none items-center justify-center p-1.5 text-gray-400 hover:text-gray-500">
                ...
                <div v-if="getShownMonth"
                    ...
                    <button v-for="(day, dayIdx) in getShownMonth" :key="day.date" type="button"
                            ...
                            :class="[
                                ...
                                (day.hasEvent || (!day.isSelected && day.isCurrentMonth && !day.isToday)) && 'text-gray-900',
