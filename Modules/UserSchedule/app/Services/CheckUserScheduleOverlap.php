...
namespace Modules\UserSchedule\Services;
        ...
        foreach ($this->schedules as $index => $schedule) {
            if ($schedule['type'] === UserScheduleRecordType::DAY_OFF->value) {
                $this->dayOffExclusions[] = [
                    'index'   => $index, // @pest-mutate-ignore
                    'id'      => Arr::get($schedule, 'id'), // @pest-mutate-ignore
                    'day_off' => Arr::get($schedule, 'day_off'), // @pest-mutate-ignore
                ];
            ...
            $this->schedulesByDay[$dayOfWeek][] = [
                'index'      => $index,
                'id'         => $schedule['id'] ?? null, // @pest-mutate-ignore
