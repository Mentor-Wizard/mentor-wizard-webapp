...
Your Laravel application has been successfully migrated from Nginx to **FrankenPHP** with **Caddy** as the web server. This setup provides:

- **FrankenPHP**: A modern PHP application server built on Go with native Laravel support
- **Caddy**: An automatic HTTPS web server with built-in SSL certificate management
...
- **Development**: `dunglas/frankenphp:1.3.6-php8.4-alpine` with Xdebug extension
...
2. **Access the application**:
   - **HTTPS** (recommended): https://localhost (may show certificate warning - this is normal for self-signed certificates)
...
Your application is now running on a modern, efficient FrankenPHP + Caddy stack with automatic SSL support!
