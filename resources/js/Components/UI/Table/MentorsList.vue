...
<template>
    <div class="justify-center" v-if="mentors['data'] && mentors['data'].length">
        <ul role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            <li v-for="mentor in mentors['data']" :key="mentor.email"
                ...
                <div class="flex flex-1 flex-col p-8">
                    ...
                    <dl class="mt-1 flex grow flex-col justify-between">
                        ...
                        <dd class="text-sm text-gray-500 ">{{ mentor?.profile?.description }}</dd>
                    ...
                    <div class="mt-px flex divide-x divide-gray-200">
                        <div class="flex w-0 flex-1">
                            ...
                            <a :href="`${mentor.profile.linkedin}`" v-if="mentor?.profile?.linkedin"
                               class="relative -mr-px inline-flex w-0 flex-1 items-center justify-center bg-blue-100 gap-x-3 rounded-br-lg border border-transparent py-4 text-sm font-semibold text-gray-900">
