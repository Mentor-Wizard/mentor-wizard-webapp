<script setup>
import {Listbox, ListboxButton, ListboxOption, ListboxOptions, Tab, TabList} from "@headlessui/vue";
    ...
    <Listbox as="div" v-model="selected">
      <div class="relative mt-2">
        <ListboxButton
          ...
          <span class="col-start-1 row-start-1 truncate pr-6">{{ options[selected]?.name ?? '' }}</span>
          <ChevronUpDownIcon
        ...
        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
          ...
          <TabList>
            <ListboxOptions
              ...
              <Tab :as="ListboxOption"
                   v-for="(item, key) in options" :key="key" v-slot="{ selected }">
                <div
                    :class="[selected ? 'bg-indigo-600 text-white outline-hidden' : 'text-gray-900', 'relative cursor-default py-2 pr-9 pl-3 select-none']">
