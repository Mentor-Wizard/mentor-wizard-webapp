...
**Прості value/label Enum-класи звільнені від вимоги `mutates()`.** Це
стосується enum-ів, що є лише таблицею відповідностей — тонкі обгортки навколо
`match`, `from()`, `names()`, `values()` без жодного умовного розгалуження. Для
них достатньо `covers()` або звичайного тесту без мутаційної директиви.

Обґрунтування: мутаційне тестування чистої lookup-таблиці породжує лише
косметичні мутанти на кшталт підміни рядка в одній гілці `match`. Pest
тривіально «вбиває» їх наявними case-by-case перевірками, але це не валідує
жодної реальної поведінки. Коректність такого enum вже гарантована: (а) нативною
системою типів backed-enum у PHP, яка не допускає невалідних кейсів, та (б)
непрямим виконанням через Feature-тести коду-споживача (наприклад,
`CalendarViewModeEnum` перевіряється через `ListCalendarEventTest`, а
`MentorSessionDurationOptionsEnum` — через тести Request-правил).
...
бізнес-результатів. Приклади: `PaymentStatusEnum` та `CalendarProviderEnum` —
вони й далі вимагають `mutates()`.

Виняток діє **виключно для Enum-класів**. Actions, DTO, Request-и та решта
категорій зберігають повну вимогу мутаційного покриття.
