...
class GetBookingCalendarEventsService
{
    ...
    public function __construct(private readonly CarbonInterface $date,
        private readonly User $user, private readonly string $timezone, private readonly ?MentorProgram $mentorProgram = null,
    ) {}
    ...
    {
        $availableSlots = (new GetAvailableSlotsService($this->user,
            $this->timezone, [], true, $this->mentorProgram)
            ->getAvailableSlots());
        $slotsCollection = collect($availableSlots)->map(fn ($slot): stdClass => (object) ($slot));
        $slotsCollection = ($slotsCollection)->each(function ($slot): void {
            $slot->date = Date::parse($slot->start)->setTimezone($this->timezone)->format('Y-m-d');
        });

        return $slotsCollection->groupBy('date')->map($this->formatDateEvents(...))->all();
    }
    ...
    private function buildCalendarView(array $monthDates, array $slots): array
    {
        foreach ($monthDates as $monthDate) {
            ...
            $this->calendarView[] = ['date' => $dateKey, 'slots' => $slotsPeriods,
    ...
    private function formatDateEvents(SupportCollection $dateSlots): array
    {
    ...
    {
        return ! $this->date->greaterThanOrEqualTo(Date::now()->addMonths(CalendarEvent::MAXIMUM_NUMBER_OF_MONTHS_EVENT_CAN_BE_SET));
    }

    private function hasFurtherSlots(): bool
    {
        return ! $this->date->lessThanOrEqualTo(Date::now()->subMonths(CalendarEvent::MAXIMUM_NUMBER_OF_MONTHS_EVENT_CAN_BE_SET));
    }
}
