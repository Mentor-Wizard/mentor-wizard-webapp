...
### Development Scripts
**IMPORTANT: ALL CONSOLE COMMANDS MUST BE EXECUTED EXCLUSIVELY IN DOCKER CONTAINERS**
- Never run console commands directly on the host system
- Always use `docker compose exec app` prefix for all commands
- This includes tests, Laravel Artisan commands, Composer, Yarn/NPM, and all development tools

### Docker Container Status Verification
**CRITICAL: Always verify Docker containers are running before executing any commands**

Before running any console commands, check if Docker containers are running:
```bash
...
If any containers show as "Exit" or are missing, start all services:
```bash
docker compose up -d
```

Wait for all containers to be in "Up" status before proceeding with development tasks.
...
- **Laravel Actions (Default)**: Business logic MUST be organized in Action classes by default (`lorisleiva/laravel-actions`)
    - **Creation**: Always use `docker compose exec app php artisan make:action` command to create Action classes
    - **Organization**: Actions are organized by domain (Auth, MentorPrograms, etc.) in `app/Actions/` directory
    - **Structure**: Action classes must implement `handle()` method; use `AsController` trait only when the action serves as a controller
    - **Naming**: Action class names should be descriptive verbs (Login, Registration, DeleteMentorProgram)
    - **Dependencies**: Action classes handle form requests, validation, and business logic
    - **Return Types**: Actions should return proper response types (RedirectResponse, JsonResponse, etc.)
    - **Testing**: Each Action class must have corresponding unit/feature tests
...
### Common Commands
**IMPORTANT: All commands must be executed in Docker containers using `docker compose exec app` prefix**
```bash
# Code quality checks
