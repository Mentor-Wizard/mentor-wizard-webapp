                ...
                <div ref="containerNav" class="sticky top-0 z-30 flex-none bg-white shadow-sm ring-1 ring-black/5 sm:pr-8">
                    <div class="grid grid-cols-7 text-sm/6 text-gray-500 sm:hidden">
                        <div v-for="(day, dayIdx) in events.calendarView">
                            <button type="button"
                                ...
                                <span class="mt-1 flex size-8 items-center justify-center font-semibold text-gray-900">{{new Date(day.date).getDate()}}</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="-mr-px hidden grid-cols-7 divide-x divide-gray-100 border-r border-gray-100 text-sm/6 text-gray-500 sm:grid">
                        <div class="col-end-1 w-14" />
                            <div v-for="(day, dayIdx) in events.calendarView">
                                <span @click = "scrollDate('exact date',day.date)"
                                      :disabled = "!day.hasEvents">{{props.weekDays[dayIdx]}} <span class="items-center justify-center font-semibold text-gray-900">
                                    {{new Date(day.date).getDate()}}</span></span>
                            </div>
                    </div>
                </div>
                    ...
                    <div class="grid flex-auto grid-cols-1 grid-rows-1">
                        ...
                        <div class="col-start-1 col-end-2 row-start-1 grid divide-y divide-gray-100" style="grid-template-rows: repeat(48, minmax(3.5rem, 1fr))">
                            <div ref="containerOffset" class="row-end-1 h-7" />
                                <div v-for="hour in hours" :key="hour">
                                    <div class="sticky left-0 z-20 -mt-2.5 -ml-14 w-14 pr-2 text-right text-xs/5 text-gray-400">{{hour}}</div>
                                </div>
                            <div />
                        </div>
                        ...
                        <ol class="col-start-1 col-end-2 row-start-1 grid grid-cols-1 sm:grid-cols-7 sm:pr-8" style="grid-template-rows: 1.75rem repeat(288, minmax(0, 1fr)) auto">
                            <template v-for="event in events.events">
                                <li :class="`relative mt-px flex sm:col-start-${event.dayNumber}`"
                                    ...
                                    <a
                                       ...
                                       :class="`group absolute inset-1 flex flex-col overflow-y-auto rounded-lg bg-${event.colour}-50 p-2 text-xs/5 hover:bg-${event.colour}-100`">
                                        <p :class="`order-1 font-semibold text-${event.colour}-700`">{{ event.title }}</p>
