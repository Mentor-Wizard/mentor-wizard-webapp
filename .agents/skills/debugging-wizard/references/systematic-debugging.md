...
> Reference for: Debugging Wizard Load when: complex bugs, multiple failed
...
Jumping to fixes without understanding causes creates more bugs. Systematic
debugging prevents the "fix one thing, break two more" cycle.
...
**Statement:** The crash occurs because `users` is undefined when the query is
complete but returns no data.
...
Three failures in different locations signals architectural problems, not
isolated bugs.
...
_Adapted from [obra/superpowers](https://github.com/obra/superpowers) by Jesse
Vincent (@obra), MIT License._
