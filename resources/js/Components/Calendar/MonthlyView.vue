        ...
        <div class="shadow-sm ring-1 ring-black/5 lg:flex lg:flex-auto lg:flex-col">
            <div
                ...
                <div v-for="weekDay in props.weekDays" class="bg-white py-2">
                    <div class="bg-white py-2">{{weekDay.slice(0,1)}}<span class="sr-only sm:not-sr-only">{{weekDay.slice(1,3)}}</span></div>
                </div>
            </div>
...
<!--                                :href="event.href"-->
                ...
                <div class="isolate grid w-full grid-cols-7 grid-rows-6 gap-px lg:hidden">
                    <button v-for="day in days.calendarView" :key="day.date" type="button"
                            @click = "scrollDate('exact date',day.date)"
                            :disabled = "!day.events || (day.events.length === 0)"
                        ...
                        <time :datetime="day.date"
                              :class="[day.isSelected && 'flex size-6 items-center justify-center rounded-full', day.isSelected && day.isToday && 'bg-indigo-600', day.isSelected && !day.isToday && 'bg-gray-900', 'ml-auto']">
                            {{ day?.date?.split('-').pop().replace(/^0/, '') }}
                        </time>
                        <span class="sr-only">{{ day.events ? day.events.length : "-" }} events</span>
                        <span v-if="day.events && (day.events.length > 0)"
